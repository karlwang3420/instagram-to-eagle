...
function core(){const c=vm.createContext({URL});vm.runInContext(source('core.js'),c);return c.EagleCore;}
...
test('removed bulk and menu entry points are not shipped or registered',()=>{
  ...
  assert.ok(!/browser\.menus|saveProfile\(/.test(source('background.js')));
  assert.ok(!/Download All|confirmAll|profile-progress/.test(source('profile-controls.js')));
  assert.ok(!/class="actions"|id="current"|id="all"|id="reel"/.test(source('popup.html')));
});
